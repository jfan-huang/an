 ...
 * 
 */
package org.jfan.an.cache.memcached;
...
public class MemcachedServiceImpl implements AddCacheService {
	...
	public <V> boolean set(String key, V value, int exp) {
		...
		try {
			...
			logger.error("Set MemCache ERROR. key: {}, exp: {}, value: {}", key, value, exp_, e);
		}
		return false;
	}
	...
	public <V> boolean add(String key, V value, int exp) {
		...
		try {
			...
			logger.error("Add MemCache ERROR, Params... key: {}, value: {}, exp: {}", key, value, exp_, e);
		}
		return false;
	}
	...
	private class ByteArrayTranscoder extends SerializingTranscoder {
		 ...
		 * 
		 * @see
		 * net.rubyeye.xmemcached.transcoders.BaseSerializingTranscoder#deserialize
		 * (byte[])
		 */
		@Override
